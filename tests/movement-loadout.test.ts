...
describe('active-weapon movement tuning', () => {
  it('applies the agreed modifiers to walking and sprinting only', () => {
    expect(movementSpeedFor('knife', false, false, false, false)).toBeCloseTo(6 * 1.08);
    expect(movementSpeedFor('knife', false, false, false, true)).toBeCloseTo(9 * 1.08);
  ...
  it('keeps aiming, sneaking and prone speeds independent of the held weapon', () => {
    ...
    expect(movementSpeedFor('knife', false, true, false, false)).toBeCloseTo(SNEAK_SPEED);
