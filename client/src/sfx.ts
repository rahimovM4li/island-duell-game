...
import { distanceAttenuation } from './surface-audio';
...
export type SfxName =
  ...
  | 'stepGrass' | 'stepStone' | 'stepSand' | 'stepWood' | 'stepMetal'
  | 'crawlGrass' | 'crawlStone' | 'crawlSand' | 'crawlWood' | 'crawlMetal' | 'bushRustle'
...
export class Sfx {
  ...
  private spatialOptions: { occluded: boolean; soft: boolean } = { occluded: false, soft: false };
  ...
  private connectSpatial(node: AudioNode, footsteps: boolean, pan: number): void {
    ...
    if (this.spatialOptions.occluded || this.spatialOptions.soft) {
      const filter = this.ctx.createBiquadFilter();
      filter.type = 'lowpass';
      filter.frequency.value = this.spatialOptions.occluded ? 820 : 1900;
      filter.Q.value = 0.65;
      tail.connect(filter);
      tail = filter;
    }
    if (this.spatialOptions.occluded) {
      const obstructionGain = this.ctx.createGain();
      obstructionGain.gain.value = 0.42;
      tail.connect(obstructionGain);
      tail = obstructionGain;
    }
    tail.connect(panner).connect(bus);
  }
  ...
  play(
    ...
    intensity = 1,
    pan = 0,
    options: { occluded?: boolean; soft?: boolean } = {},
  ): void {
    if (!this.ctx) return;
    this.spatialOptions = { occluded: options.occluded ?? false, soft: options.soft ?? false };
    const weaponSound = name === 'pistol' || name === 'rifle' || name === 'shotgun' || name === 'sniper'
      || name === 'explosion';
    const footsteps = name.startsWith('step') || name.startsWith('crawl') || name === 'bushRustle';
    const category = weaponSound ? 'weapon' : footsteps ? 'footstep' : 'effect';
    const level = distanceAttenuation(distance, category) * Math.max(0, intensity);
    if (level <= 0) return;
    ...
    switch (name) {
      ...
      case 'rifle':
        noise(0.09, hz(3200), 0.65 * level);
        if (distance > 45) tone(hz(105), 0.28, 0.08 * level, 'triangle', -35);
        break;
      case 'shotgun': noise(0.28, hz(1500), 0.95 * level); tone(hz(90), 0.2, 0.4 * level, 'square', -40); break;
      case 'sniper':
        noise(0.16, hz(2600), 1.05 * level, 0.7);
        tone(hz(70), 0.42, 0.5 * level, 'square', -35);
        noise(distance > 45 ? 0.72 : 0.5, hz(distance > 45 ? 360 : 500), 0.3 * level, 0.5);
        break;
      ...
      case 'shieldHit': noise(0.055, hz(1800), 0.18 * level, 1.7); tone(hz(310), 0.1, 0.2 * level, 'triangle', 80); break;
      case 'shieldBreak': noise(0.12, hz(2400), 0.28 * level, 1.1); tone(hz(520), 0.18, 0.23 * level, 'square', -260); break;
      case 'helmetBreak': noise(0.09, hz(1500), 0.26 * level, 1.3); tone(hz(780), 0.13, 0.18 * level, 'triangle', -320); break;
      ...
      case 'stepWood': noise(0.045, hz(1450), 0.13 * level, 1.55); tone(hz(225), 0.065, 0.075 * level, 'triangle', -45); break;
      case 'stepMetal': noise(0.035, hz(2800), 0.12 * level, 1.9); tone(hz(690), 0.075, 0.07 * level, 'triangle', -90); break;
      case 'crawlGrass': noise(0.12, hz(440), 0.11 * level, 1.25); break;
      case 'crawlSand': noise(0.15, hz(720), 0.095 * level, 1.1); break;
      case 'crawlStone': noise(0.09, hz(1150), 0.085 * level, 1.45); break;
      case 'crawlWood': noise(0.1, hz(920), 0.09 * level, 1.35); tone(hz(170), 0.08, 0.035 * level, 'triangle', -30); break;
      case 'crawlMetal': noise(0.085, hz(1700), 0.08 * level, 1.5); tone(hz(410), 0.07, 0.035 * level, 'triangle', -70); break;
